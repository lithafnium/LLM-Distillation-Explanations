...
def combine_exp_res(dict_bert, dict_distilbert, dict_mobilebert, dict_tinybert, exp_type):
    N = len(dict_bert)
    res_cos_sim = OrderedDict(
        {"distilbert": [], "mobilebert": [], "tinybert": []})
    res_l2_dist = OrderedDict(
        {"distilbert": [], "mobilebert": [], "tinybert": []})
    ...
    for i in range(N):
        ...
        if exp_type == 'lime':
            ...
            distilbert_attr_i = np.array(
                [distilbert_map_i[ft] if ft in distilbert_map_i else 0 for ft in features_i])
            mobilebert_map_i = tk_attr_map(dict_mobilebert, i)
            mobilebert_attr_i = np.array(
                [mobilebert_map_i[ft] if ft in mobilebert_map_i else 0 for ft in features_i])
            tinybert_map_i = tk_attr_map(dict_tinybert, i)
            tinybert_attr_i = np.array(
                [tinybert_map_i[ft] if ft in tinybert_map_i else 0 for ft in features_i])
        elif exp_type == 'shap':
            distilbert_attr_i = np.array(
                dict_distilbert[f'{i}']['attributions'])
            mobilebert_attr_i = np.array(
                dict_mobilebert[f'{i}']['attributions'])
            tinybert_attr_i = np.array(dict_tinybert[f'{i}']['attributions'])
        elif exp_type == 'int_grad':
            distilbert_attr_i = np.array(
                dict_distilbert[f'{i}']['attribution'])
            mobilebert_attr_i = np.array(
                dict_mobilebert[f'{i}']['attribution'])
...
def grapher(attr_dict, task, exp_type, metric):
    ...
    plt.title(
        f"{EXP_NAME_MAPPING[exp_type]} {METRIC_NAME_MAPPING[metric]}, student v.s. BERT: {TASK_NAME_MAPPING[task]} Task")
    # plt.show()
    if not os.path.exists(f"{folder}/comparison_plots/{task}"):
        os.makedirs(f"{folder}/comparison_plots/{task}")
    plt.savefig(
        f"{folder}/comparison_plots/{task}/{task}_{exp_type}_{metric}.png")
    plt.close()


def record_statistics(attr_dict, task, exp_type, metric, file):
    distilbert_avg, distilbert_median = np.nanmean(
        attr_dict['distilbert']), np.nanmedian(attr_dict['distilbert'])
    mobilebert_avg, mobilebert_median = np.nanmean(
        attr_dict['mobilebert']), np.nanmedian(attr_dict['mobilebert'])
    tinybert_avg, tinybert_median = np.nanmean(
        attr_dict['tinybert']), np.nanmedian(attr_dict['tinybert'])
    ...
    with open(file, 'a') as f:
        ...
        f.write(
            f"Distilbert {METRIC_NAME_MAPPING[metric]} - AVG {distilbert_avg:.5f}, MEDIAN {distilbert_median:.5f}, N {N_distilbert}\n")
        f.write(
            f"Mobilebert {METRIC_NAME_MAPPING[metric]} - AVG {mobilebert_avg:.5f}, MEDIAN {mobilebert_median:.5f}, N {N_mobilebert}\n")
        f.write(
            f"Tinybert {METRIC_NAME_MAPPING[metric]} - AVG {tinybert_avg:.5f}, MEDIAN {tinybert_median:.5f}, N {N_tinybert}\n\n")
...
def main():
    for exp_type in exp_types:
        for task in tasks:
            ...
            bert_res = json.load(
                open(f'{folder}/{baseline}/{baseline}_{task}_{exp_type}.json'))
            distilbert_res = json.load(
                open(f'{folder}/{student_models[0]}/{student_models[0]}_{task}_{exp_type}.json'))
            mobilebert_res = json.load(
                open(f'{folder}/{student_models[1]}/{student_models[1]}_{task}_{exp_type}.json'))
            tinybert_res = json.load(
                open(f'{folder}/{student_models[2]}/{student_models[2]}_{task}_{exp_type}.json'))

            combined_res = combine_exp_res(
                bert_res, distilbert_res, mobilebert_res, tinybert_res, exp_type)
            for metric in metrics:
                record_statistics(
                    combined_res[metric], task, exp_type, metric, "explanation_results/comparison.txt")
